...
never looks merged by merge-base alone.
    ...
    gh poi -b main                 # check merged/closed status against a base other than the default
...
New. Wired via `core.pager` (a `.gitconfig.local`-friendly hook, falling back
...
New. Wired as `git dft`, a `difftool -t difftastic` alias — deliberately NOT
...
New. `git absorb --and-rebase` looks at your worktree's uncommitted hunks,
...
New. `jj git init --colocate` in an existing checkout keeps `.git` as the
...
New. Run from this repo's own `.pre-commit-config.yaml`, and worth running
...
The `gzg` binary (`cargo install ghzinga`), configured in `config/ghzinga/`.
