...
install_tools_macos() {
  ...
  for f in starship zellij zoxide atuin fzf eza bat tmux; do brew_ensure "$f"; done
...
ensure_zellij_linux() {
  if ! command -v cargo >/dev/null 2>&1; then
    echo "cargo not found — installing rustup (zellij has no apt package)"
  ...
  current=""
  command -v zellij >/dev/null 2>&1 && current="$(zellij --version | awk '{print $2}')"
  latest="$(curl -fsSL -H "User-Agent: dotfiles-install-script (github.com/andyhite/dotfiles)" \
    https://crates.io/api/v1/crates/zellij | grep -o '"newest_version":"[^"]*"' | head -1 | cut -d'"' -f4)"

  if [ -n "$current" ] && [ "$current" = "$latest" ]; then
    echo "zellij: up to date ($current)"
    return
  fi
  echo "zellij: ${current:+updating $current -> }${current:-installing }$latest (compiling — this takes a few minutes)"
...
install_tools_linux() {
  if command -v apt-get >/dev/null 2>&1; then
    ...
    for p in zoxide eza bat tmux unzip; do apt_ensure "$p"; done
  else
    echo "no apt-get found — skipping distro packages (zoxide/eza/bat/tmux); install manually"
  fi
